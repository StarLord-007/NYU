...
@dataclass
class SVMConfig(BaseAltConfig):
    params: dict = field(default_factory=lambda: dict(DEFAULT_PARAMS))


class SVMModel(BaseAltSklearnModel):
    supports_sample_weight = True
    threshold_weighted = False

    def _build_estimator(self, y: np.ndarray):
        return SVC(probability=True, random_state=self.cfg.random_state, **dict(self.cfg.params))


def factory_from_cfg(cfg: SVMConfig):
    return lambda cfg=cfg: SVMModel(cfg)


def build_registry(base_cfg: SVMConfig, tuned_params: dict, random_state: int) -> dict[str, SVMConfig]:
    tuned_cfg = SVMConfig(
...
def plot_figures(folds: pd.DataFrame, summary: pd.DataFrame, lopo: pd.DataFrame | None, out_dir: Path) -> None:
    ...
    ax.set_title("SVM unseen-paper discrimination\n(blue = individual folds, red = mean +/- sd)")
    ...
    if lopo is not None:
        ...
        ax.set_title("SVM LOPO baseline vs group-tuned\n(marker size = paper size)")
        plt.tight_layout()
...
def main() -> None:
    ...
    ap.add_argument("--out", default=str(Path(__file__).resolve().parent / "svm_outputs"))
    ...
    if len(df) > 5000:
        warnings.warn("SVM on >5000 rows may be slow; consider a linear kernel or subsampling.")
    ...
    for name, cfg in registry.items():
        folds = benchmark_on_splits(
            ...
            df,
            outer_splits,
            threshold_weighted=SVMModel.threshold_weighted,
        )
    ...
    final = SVMModel(registry["group-tuned"])
    final.fit(df, y, papers)
    joblib.dump(final, out_dir / "svm_model.joblib")
    with open(out_dir / "svm_model_card.json", "w") as handle:
        json.dump(
            {
