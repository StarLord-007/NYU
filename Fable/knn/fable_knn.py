...
@dataclass
class KNNConfig(BaseAltConfig):
    params: dict = field(default_factory=lambda: dict(DEFAULT_PARAMS))


class KNNModel(BaseAltSklearnModel):
    threshold_weighted = True

    def _build_estimator(self, y: np.ndarray):
        return KNeighborsClassifier(n_jobs=-1, **dict(self.cfg.params))


def factory_from_cfg(cfg: KNNConfig):
    return lambda cfg=cfg: KNNModel(cfg)


def build_registry(base_cfg: KNNConfig, tuned_params: dict, random_state: int) -> dict[str, KNNConfig]:
    tuned_cfg = KNNConfig(
...
def plot_figures(folds: pd.DataFrame, summary: pd.DataFrame, lopo: pd.DataFrame | None, out_dir: Path) -> None:
    ...
    ax.set_title("KNN unseen-paper discrimination\n(blue = individual folds, red = mean +/- sd)")
    ...
    if lopo is not None:
        ...
        ax.set_title("KNN LOPO baseline vs group-tuned\n(marker size = paper size)")
        plt.tight_layout()
...
def main() -> None:
    ...
    ap.add_argument("--out", default=str(Path(__file__).resolve().parent / "knn_outputs"))
    ...
    for name, cfg in registry.items():
        folds = benchmark_on_splits(
            ...
            df,
            outer_splits,
            threshold_weighted=KNNModel.threshold_weighted,
        )
    ...
    final = KNNModel(registry["group-tuned"])
    final.fit(df, y, papers)
    joblib.dump(final, out_dir / "knn_model.joblib")
    with open(out_dir / "knn_model_card.json", "w") as handle:
        json.dump(
            {
