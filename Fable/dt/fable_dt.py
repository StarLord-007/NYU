...
@dataclass
class DTConfig(BaseAltConfig):
    params: dict = field(default_factory=lambda: dict(DEFAULT_PARAMS))


class DTModel(BaseAltSklearnModel):
    supports_sample_weight = True
    threshold_weighted = False

    def _build_estimator(self, y: np.ndarray):
        return DecisionTreeClassifier(random_state=self.cfg.random_state, **dict(self.cfg.params))


def factory_from_cfg(cfg: DTConfig):
    return lambda cfg=cfg: DTModel(cfg)


def build_registry(base_cfg: DTConfig, tuned_params: dict, random_state: int) -> dict[str, DTConfig]:
    tuned_cfg = DTConfig(
...
def plot_figures(folds: pd.DataFrame, summary: pd.DataFrame, lopo: pd.DataFrame | None, out_dir: Path) -> None:
    ...
    ax.set_title("Decision Tree unseen-paper discrimination\n(blue = individual folds, red = mean +/- sd)")
    ...
    if lopo is not None:
        ...
        ax.set_title("Decision Tree LOPO baseline vs group-tuned\n(marker size = paper size)")
        plt.tight_layout()
...
def main() -> None:
    ...
    ap.add_argument("--out", default=str(Path(__file__).resolve().parent / "dt_outputs"))
    ...
    if args.tune:
        print("Group-aware Decision Tree search ...")
    ...
    for name, cfg in registry.items():
        folds = benchmark_on_splits(
            ...
            df,
            outer_splits,
            threshold_weighted=DTModel.threshold_weighted,
        )
    ...
    final = DTModel(registry["group-tuned"])
    final.fit(df, y, papers)
    joblib.dump(final, out_dir / "dt_model.joblib")
    with open(out_dir / "dt_model_card.json", "w") as handle:
        json.dump(
            {
