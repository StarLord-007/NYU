...
(same 22 fields), but the raw cell contents introduce a few new quirks
that we now handle:

* **Oxygen concentration** now comes in two formats in the same column:
  percent strings (``"21.0%"``, ``"50%"``) AND already-normalised mole
  fractions (``"0.21"``, ``"0.50"``). The same value range therefore
  spans two encodings, so we parse a number and divide by 100 only when
  it is > 1, otherwise keep it as a fraction.
* **Geometry / Ignition method** columns now contain mixed-case duplicates
  ("Flat" vs "flat", "Open Flame" vs "open flame"). We normalise to a
  small canonical set using case-insensitive matching.
* **Pressure** sometimes appears as a bare number (e.g. ``413.2``) with
  no unit. We treat unit-less numbers as kPa, which is consistent with
  the unit used by ~85 % of labelled rows.
* **Gravity** now uses the scientific-notation token ``1e-05`` for
  micro-gravity that our regex must accept (it does, the original regex
  already supported scientific notation).
* **Class balance** has shifted slightly from 70/30 to 76/24 (ignition
  is more dominant), so ``scale_pos_weight = neg/pos`` is recomputed
  from the data itself rather than hardcoded.

Everything below — cleaning logic, leakage protection, XGBoost rationale,
evaluation strategy, artifacts — mirrors v1 so the two scripts are
directly comparable. The detailed justifications are kept inline.

Why XGBoost for this problem
----------------------------
* Tabular data with mixed numeric/categorical features and strong
  non-linear interactions (O2, pressure, gravity, flow, material) →
  gradient-boosted decision trees dominate this regime.
* Handles missing values natively — no need to fabricate imputations for
  predictors that are physically not applicable (e.g. ``core_diameter_mm``
  on a flat sample).
* Tree splits are invariant to monotone transforms, so we only need
  consistent *units* across rows; no scaling/log-transform required.
* Built-in L1/L2 regularisation, row/column subsampling and
  ``scale_pos_weight`` give us strong control over over-fitting and
  class imbalance on a dataset of this size (~5 k rows).
* Exposes gain-based feature importance for interpretation by a
  combustion-physics audience.

Evaluation strategy
-------------------
* 80/20 stratified hold-out for the headline numbers.
* 5-fold stratified CV on the full dataset.
* 5-fold ``GroupKFold`` keyed on source article/DOI — the honest test of
  cross-campaign generalisation when the data is a literature
  aggregation.
* Threshold tuning on the hold-out probability curve to maximise F1
  (in addition to the default 0.5).

Outputs (written to ``artifacts_v2/``)
--------------------------------------
* ``xgb_ignition_model_v2.joblib`` — full fitted pipeline.
* ``feature_importance.csv``        — gain-based importances.
* ``confusion_matrix.png``          — confusion matrix on hold-out.
* ``roc_curve.png``                 — ROC curve on hold-out.
* ``metrics.json``                  — all numerical scores.

Run::

    python xgb_ignition_model_2.py --data database_xgb.csv
    python xgb_ignition_model_2.py --data database_xgb.csv --tune
"""
...
# Accepts integer, decimal, and scientific-notation tokens (e.g. ``1e-05``).
...
def parse_oxygen_fraction(x):
    """Return O2 as a mole fraction in [0, 1].

    The v2 database mixes ``"21.0%"`` and ``"0.21"`` style entries in the
    same column. We parse a number first and only divide by 100 when it
    is > 1 (i.e. clearly a percentage).
    """
    v = _first_number(x)
    if pd.isna(v):
        return np.nan
...
def parse_pressure_kpa(x):
    ...
    Bare numbers (no unit) are assumed to be kPa — that is the unit used
    by the large majority of explicitly-labelled rows in the v2 file
    (``101 kPa``, ``56.5 kPa``, etc.).
    """
    v = _first_number(x)
    if pd.isna(v):
        return np.nan
...
def extract_dimensions_mm(text):
    """Pull every length out of a dimension string and return them in mm."""
    if pd.isna(text):
        return []
...
def normalise_geometry(x):
    """Collapse case/typo variants ('Flat', 'flat', 'Cylindrical (Hollow)')."""
    s = _clean_text(x)
    if pd.isna(s):
        return "Unknown"
    l = str(s).lower()
...
def normalise_ignition_method(x):
    """Collapse 'Open Flame' / 'Open flame' / 'open flame' to one bucket."""
    s = _clean_text(x)
    if pd.isna(s):
        return "Unknown"
    l = str(s).lower()
...
def normalise_yes_no(x):
    """'Yes'/'No' (any case, trailing whitespace) -> 1/0/NaN."""
    s = _clean_text(x)
    if pd.isna(s):
        return np.nan
    l = str(s).lower().strip()
...
# Canonical column names in the v2 CSV. The first physical row is a section
# banner ("Citation", "Sample", ...) so we skip it. The column *names* are
...
# Columns observed *after* the ignition decision — must NOT be used as
# predictors, or the model would trivially leak the target.
POST_IGNITION_LEAKS = [
    "Flame Length",
...
def load_clean(data_path: Path) -> pd.DataFrame:
    ...
    # Drop rows without a label and exact-duplicate rows.
...
# Numeric vs. categorical lists consumed by the ColumnTransformer.
NUMERIC_FEATURES = [
    "oxygen_fraction",
    "pressure_kpa",
    "flow_velocity_mm_s",
...
def build_pipeline(scale_pos_weight: float, random_state: int = 42) -> Pipeline:
    ...
    * Categorical branch: one-hot. We impute the literal string ``"Unknown"``
      so missing-categoricals become their own bin and
      ``handle_unknown='ignore'`` gracefully handles unseen categories at
      inference time.
    """
...
def _save_confusion_matrix(y_true, y_pred, path: Path) -> None:
    ...
    ax.set_ylabel("True")
    ax.set_title("Confusion matrix (hold-out, v2)")
...
def _save_roc(y_true, y_score, path: Path) -> None:
    ...
    ax.set_title("ROC curve (hold-out, v2)")
...
def main() -> None:
    ...
    parser.add_argument(
        "--data", default="database_xgb.csv", help="Path to the v2 database CSV."
    )
    parser.add_argument(
        "--out", default="artifacts_v2", help="Directory to write artifacts to."
    )
    parser.add_argument(
        "--tune",
        action="store_true",
        help="Run RandomizedSearchCV (slower, ~a few minutes).",
    )
    parser.add_argument(
    ...
    pos = int((y == 1).sum())
    neg = int((y == 0).sum())
    # ``scale_pos_weight`` reweights the positive class so XGBoost's gradient
    # is balanced. Convention: neg/pos. Here ignition is the majority class,
    # so this value will be < 1 and XGBoost will down-weight the easy positive
    # class, focusing on the harder no-ignition rows. We recompute it from
    # the v2 data (not the v1 hardcoded value) because the class balance has
    # shifted from ~70/30 to ~76/24.
    spw = neg / max(pos, 1)
    ...
    joblib.dump(pipe, out_dir / "xgb_ignition_model_v2.joblib")
