...
BOOTSTRAP_METRICS = ("roc_auc", "pr_auc", "mcc", "balanced_accuracy")
...
def bootstrap_metric_ci(
    ...
    """Non-parametric bootstrap CI for the mean of one metric vector."""
    ...
    if len(values) == 1 or n_bootstrap <= 0:
        v = float(values[0])
        return {"mean": v, "ci_low": v, "ci_high": v, "n": int(len(values))}
...
def bootstrap_folds(
    ...
    """Bootstrap fold-level means per model for one benchmark strategy."""
    rows = []
    for model_i, (method, sub) in enumerate(folds.groupby("method", sort=False)):
        for metric_i, metric in enumerate(metrics):
            ci = bootstrap_metric_ci(
                sub[metric].to_numpy(),
                n_bootstrap=n_bootstrap,
                random_state=random_state + 1009 * model_i + 37 * metric_i,
            )
            rows.append(
                {
                    "strategy": strategy,
...
def bootstrap_lopo_pooled(
    lopo_predictions: dict[str, pd.DataFrame],
    n_bootstrap: int = 2000,
    random_state: int = RNG,
) -> pd.DataFrame:
    """Cluster bootstrap LOPO pooled metrics by resampling papers."""
    rows = []
    for model_i, (method, pred) in enumerate(lopo_predictions.items()):
        papers = pred["paper_id"].to_numpy()
        unique_papers = pd.unique(papers)
        rng = np.random.default_rng(random_state + 7919 * model_i)
        draws = {"pooled_roc_auc": [], "pooled_pr_auc": []}
        for _ in range(n_bootstrap):
            chosen = rng.choice(unique_papers, size=len(unique_papers), replace=True)
            idx = np.concatenate([np.flatnonzero(papers == p) for p in chosen])
            yb = pred["y_true"].to_numpy()[idx]
            pb = pred["y_score"].to_numpy()[idx]
            if len(np.unique(yb)) < 2:
                continue
            draws["pooled_roc_auc"].append(roc_auc_score(yb, pb))
            draws["pooled_pr_auc"].append(average_precision_score(yb, pb))
        ...
        for metric, values in draws.items():
            ...
            if len(values):
                rows.append(
                    {
                        "strategy": "extrapolation_lopo",
                        "method": method,
                        "metric": metric,
                        "mean": float(values.mean()),
                        "ci_low": float(np.percentile(values, 2.5)),
                        "ci_high": float(np.percentile(values, 97.5)),
                        "n": int(len(unique_papers)),
                        "bootstrap_iterations": int(n_bootstrap),
                        "valid_bootstrap_iterations": int(len(values)),
                    }
                )
            else:
                rows.append(
...
def bootstrap_markdown_table(
    boot: pd.DataFrame,
    metrics: tuple[str, ...] = BOOTSTRAP_METRICS,
) -> str:
    """Render bootstrap summary as mean [95% CI] columns."""
    labels = {
        "roc_auc": "ROC-AUC",
        "pr_auc": "PR-AUC",
        "mcc": "MCC",
    ...
    if boot is None or boot.empty:
        return "_Bootstrap summary not available._"
    pivot = boot.set_index(["method", "metric"]).sort_index()
    ordered_methods = (
        boot[boot["metric"] == metrics[0]]
        .sort_values("mean", ascending=False)["method"]
        .tolist()
    )
    lines = [
        "| Model | " + " | ".join(labels.get(m, m) for m in metrics) + " |",
        "|---" + "|---" * len(metrics) + "|",
    ]
    for method in ordered_methods:
        ...
        for metric in metrics:
            try:
                r = pivot.loc[(method, metric)]
                cells.append(f"{r['mean']:.3f} [{r['ci_low']:.3f}, {r['ci_high']:.3f}]")
            except KeyError:
                cells.append("NA")
...
def main() -> None:
    ...
    if not args.no_bootstrap:
        interp_random_boot = bootstrap_folds(
            interp_random,
            "interpolation_random_80_20",
            n_bootstrap=args.bootstrap_iters,
            random_state=args.random_state,
        )
        interp_kfold_boot = bootstrap_folds(
            interp_kfold,
            "interpolation_stratified_5fold",
            n_bootstrap=args.bootstrap_iters,
            random_state=args.random_state + 11,
        )
        interp_random_boot.to_csv(
            out_dir / "interpolation_random_bootstrap_summary.csv", index=False
        )
        interp_kfold_boot.to_csv(
            out_dir / "interpolation_stratified_bootstrap_summary.csv", index=False
        )
        bootstrap_outputs["interpolation_random"] = interp_random_boot
        bootstrap_outputs["interpolation_stratified"] = interp_kfold_boot
    ...
    if not args.no_bootstrap:
        grouped_boot = bootstrap_folds(
            grouped,
            "extrapolation_grouped_cv",
            n_bootstrap=args.bootstrap_iters,
            random_state=args.random_state + 23,
        )
        grouped_boot.to_csv(out_dir / "extrapolation_grouped_bootstrap_summary.csv", index=False)
        bootstrap_outputs["extrapolation_grouped"] = grouped_boot
    ...
    if not (args.quick or args.skip_lopo):
        ...
        for i, (name, (factory, _)) in enumerate(registry.items()):
            ...
            per_paper, pooled, oof = lopo_evaluate(name, factory, df)
            lopo_frames.append(per_paper)
            lopo_pooled[name] = pooled
            lopo_predictions[name] = pd.DataFrame(
                {
                    "method": name,
                    "paper_id": df["paper_id"].to_numpy(),
                    "y_true": df["ignition_binary"].to_numpy(),
                    "y_score": oof,
                }
            )
        ...
        if not args.no_bootstrap:
            lopo_boot = bootstrap_lopo_pooled(
                lopo_predictions,
                n_bootstrap=args.bootstrap_iters,
                random_state=args.random_state + 31,
            )
            lopo_boot.to_csv(out_dir / "lopo_pooled_bootstrap_summary.csv", index=False)
            bootstrap_outputs["lopo_pooled"] = lopo_boot
    ...
    if "interpolation_random" in bootstrap_outputs:
        lines += [
            "",
            "#### Bootstrap: random stratified 80/20",
            "",
            bootstrap_markdown_table(bootstrap_outputs["interpolation_random"]),
        ]
    lines += [
        "",
        "### Stratified 5-fold",
    ...
    if "interpolation_stratified" in bootstrap_outputs:
        lines += [
            "",
            "#### Bootstrap: stratified 5-fold",
            "",
            bootstrap_markdown_table(bootstrap_outputs["interpolation_stratified"]),
        ]
    lines += [
        "",
        "## Extrapolation",
    ...
    if "extrapolation_grouped" in bootstrap_outputs:
        lines += [
            "",
            "#### Bootstrap: repeated grouped CV",
            "",
            bootstrap_markdown_table(bootstrap_outputs["extrapolation_grouped"]),
        ]
    if lopo_pooled:
    ...
    if "lopo_pooled" in bootstrap_outputs:
        lines += [
            "",
            "#### Bootstrap: Leave-One-Paper-Out pooled",
            "",
            bootstrap_markdown_table(
                bootstrap_outputs["lopo_pooled"],
                metrics=("pooled_roc_auc", "pooled_pr_auc"),
            ),
        ]
        ...
        json.dump(
            {
                ...
                "bootstrap": {
                    k: v.to_dict(orient="records") for k, v in bootstrap_outputs.items()
                },
                "bootstrap_iterations": 0 if args.no_bootstrap else int(args.bootstrap_iters),
