...
Outputs -> ``<this script dir>/eval_outputs/``:
  * ``comparison_table.csv`` / ``comparison_table.md``
  * ``protocol_comparison.csv``  (random vs stratified vs grouped vs LOPO)
    * ``protocol_comparison.md``   (interpolation vs extrapolation sections)
  * ``lopo_baseline_vs_best.csv``
  * publication figures (``fig_*.png``)

Run::

        python fable_eval.py --data ../Microgravity_Database_reduced.csv
        python fable_eval.py --quick          # smoke test (1 seed, fewer methods)
"""
...
def _as_factory(model_or_cfg):
    """Accept either a legacy FableConfig or a zero-arg model factory.

    This is the single hook that makes the evaluation harness model-agnostic:
    any object exposing .fit(X, y, papers) / .predict_proba(X) can now be
    benchmarked, without special cases for XGBoost.
    """
    if callable(model_or_cfg):
        return model_or_cfg
    return lambda: FableModel(model_or_cfg)
...
def _inner_thresholds(
    ...
    factory = _as_factory(model_or_cfg)
...
def benchmark_method(
    ...
    for seed in range(n_seeds):
        ...
        for fold, (tr, te) in enumerate(outer.split(X, y, groups=papers)):
            ...
            thr = _inner_thresholds(factory, Xtr, ytr, ptr, seed=RNG + seed)
            model = factory()
...
def protocol_comparison(df: pd.DataFrame, model_or_cfg, out_dir: Path) -> pd.DataFrame:
    ...
    interp = prot[prot["protocol"].isin([
        "random stratified 80/20 hold-out",
        "stratified 5-fold CV",
    ])]
    extrap = prot[~prot["protocol"].isin(interp["protocol"])]
    ...
    for _, r in interp.iterrows():
        lines.append(f"| {r['protocol']} | {r['roc_auc_mean']:.3f} | {r['roc_auc_std']:.3f} |")
    ...
    (out_dir / "protocol_comparison.md").write_text("\n".join(lines) + "\n")
    return prot
