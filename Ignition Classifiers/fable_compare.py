...
Run:
    python fable_compare.py --data ../Microgravity_Database_reduced.csv

The full pipeline also produces SHAP explanations, raw-feature permutation
importance, per-paper ROC-AUC histograms, and a grouped-CV comparison of paper
...
Outputs -> ``<this script dir>/compare_outputs/``:
    ...
    permutation_importance.csv / fig_permutation_importance.png
    shap_ranking.csv / fig_shap_summary.png / fig_shap_bar.png
    paper_weighting_folds.csv / paper_weighting_summary.csv
...
from fable_train import FableConfig, FableModel, GROUP_TUNED_PARAMS

DEFAULT_DATA = Path(__file__).resolve().parents[1] / "Microgravity_Database_reduced.csv"
PAPER_WEIGHT_STRATEGIES = ("none", "inverse", "sqrt", "effective", "log")
...
# ---------------------------------------------------------------------------
# Explainability and paper-weighting audit
# ---------------------------------------------------------------------------

def _feature_names(model: FableModel) -> list[str]:
    """Return transformed feature names in the exact order seen by XGBoost."""
    return [str(name).split("__", 1)[-1] for name in model.pre.get_feature_names_out()]
...
def run_permutation_importance(
    ...
    """Permutation importance on held-out papers using ROC-AUC degradation."""
    baseline = roc_auc_score(y_test, model.predict_proba(X_test))
    ...
    for feature in model.features:
        ...
        for _ in range(n_repeats):
            ...
            drops.append(baseline - roc_auc_score(y_test, model.predict_proba(shuffled)))
        rows.append(
            {
                "feature": feature,
                "importance_mean": float(np.mean(drops)),
                "importance_std": float(np.std(drops, ddof=1)),
                "baseline_roc_auc": float(baseline),
            }
        )
    result = pd.DataFrame(rows).sort_values("importance_mean", ascending=False)
    result.to_csv(out_dir / "permutation_importance.csv", index=False)

    top = result.head(20).sort_values("importance_mean")
    fig, ax = plt.subplots(figsize=(8, max(4, 0.35 * len(top))))
    ax.barh(top["feature"], top["importance_mean"], xerr=top["importance_std"], color="#27598e")
    ax.axvline(0, color="k", lw=0.7)
    ax.set_xlabel("decrease in held-out-paper ROC-AUC")
    ax.set_title("XGBoost raw-feature permutation importance")
    plt.tight_layout()
    plt.savefig(out_dir / "fig_permutation_importance.png", bbox_inches="tight")
    plt.close(fig)
    return result
...
def run_shap_analysis(
    ...
    """Explain XGBoost predictions for a held-out-paper fold with TreeSHAP."""
    try:
        import shap
    except ImportError as exc:
        print(f"[SHAP] dependency unavailable ({exc}); skipping SHAP outputs.")
        return None

    transformed = model.pre.transform(X_test[model.features]).astype(np.float32)
    rng = np.random.default_rng(random_state)
    if len(transformed) > max_samples:
        transformed = transformed[rng.choice(len(transformed), max_samples, replace=False)]
    names = _feature_names(model)
    explainer = shap.TreeExplainer(model.models_[0])
    ...
    plt.title("XGBoost SHAP summary on held-out papers")
    plt.tight_layout()
    plt.savefig(out_dir / "fig_shap_summary.png", bbox_inches="tight")
    plt.close()
    shap.summary_plot(
        values, transformed, feature_names=names, plot_type="bar", show=False, max_display=20
    )
    plt.title("XGBoost mean absolute SHAP value")
    plt.tight_layout()
    plt.savefig(out_dir / "fig_shap_bar.png", bbox_inches="tight")
    plt.close()
    return ranking


def compare_paper_weighting(
    df: pd.DataFrame,
    splits: list[tuple[int, int, np.ndarray, np.ndarray]],
    out_dir: Path,
    random_state: int,
) -> pd.DataFrame:
    """Compare XGBoost paper weights on identical held-out-paper folds."""
    frames = []
    for strategy in PAPER_WEIGHT_STRATEGIES:
        cfg = FableConfig(
            feature_set="all",
            paper_weight=strategy,
            class_weight="balanced",
            params=dict(GROUP_TUNED_PARAMS),
            random_state=random_state,
        )
        factory = lambda cfg=cfg: FableModel(cfg)
        print(f"[paper weighting] {strategy} ...")
        frames.append(
            benchmark_interpolation(
                strategy, factory, df, splits, None, base_seed=random_state
            )
        )
    folds = pd.concat(frames, ignore_index=True)
    summary = summarise_folds(folds)
    folds.to_csv(out_dir / "paper_weighting_folds.csv", index=False)
    summary.to_csv(out_dir / "paper_weighting_summary.csv")

    chart = summary.sort_values("group_roc_auc")
    fig, ax = plt.subplots(figsize=(7, 3.8))
    ax.barh(
        chart.index,
        chart["group_roc_auc"],
        xerr=chart["group_roc_auc_std"],
        color="#27598e",
        capsize=3,
    )
    ax.axvline(0.5, color="k", ls=":", lw=0.8)
    ax.set_xlabel("held-out-paper ROC-AUC (mean +/- SD)")
    ax.set_title("XGBoost paper-weighting strategy comparison")
    plt.tight_layout()
    plt.savefig(out_dir / "fig_paper_weighting_comparison.png", bbox_inches="tight")
    plt.close(fig)
    return summary
...
def figures(folds, summary, lopo_all, lopo_pooled, out_dir: Path) -> None:
    ...
    if lopo_all is not None and len(lopo_all):
        ...
        # 4. Requested per-paper ROC-AUC histogram, overlaid by classifier.
        fig, ax = plt.subplots(figsize=(7, 4.2))
        bins = np.linspace(0, 1, 21)
        for name in names:
            vals = lopo_all.loc[lopo_all["method"] == name, "auc"].dropna()
            ax.hist(vals, bins=bins, alpha=0.35, label=name)
        ax.axvline(0.5, color="k", ls=":", lw=0.8)
        ax.set_xlabel("within-paper ROC-AUC (LOPO)")
        ax.set_ylabel("number of papers")
        ax.set_title("Per-paper ROC-AUC distribution by classifier")
        ax.legend(fontsize=7)
        plt.tight_layout()
        plt.savefig(out_dir / "fig_per_paper_roc_auc_histogram.png", bbox_inches="tight")
        plt.close(fig)
...
def main() -> None:
    ...
    ap.add_argument("--no-shap", action="store_true", help="Skip the SHAP stage.")
    args = ap.parse_args()
    ...
    # ---- paper weighting on the exact same grouped folds -------------------
    weighting_summary = compare_paper_weighting(
        df, grouped_splits, out_dir, args.random_state
    )
    md_weighting = to_markdown_table(weighting_summary)
    print("\nPaper-weighting strategy comparison:\n" + md_weighting + "\n")

    # ---- held-out-paper explainability for the XGBoost champion ------------
    _, _, explain_train, explain_test = grouped_splits[0]
    explain_model = xgb_champion_factory(args.random_state)()
    explain_model.fit(
        df.iloc[explain_train],
        df["ignition_binary"].to_numpy()[explain_train],
        df["paper_id"].iloc[explain_train],
    )
    permutation = run_permutation_importance(
        explain_model,
        df.iloc[explain_test],
        df["ignition_binary"].to_numpy()[explain_test],
        out_dir,
        args.random_state,
    )
    shap_ranking = None if args.no_shap else run_shap_analysis(
        explain_model, df.iloc[explain_test], out_dir, args.random_state
    )
    ...
    lines = [
        ...
        md_grouped,
        "",
        "### XGBoost paper-weighting strategy comparison",
        "",
        md_weighting,
        "",
        "### Explainability",
        "",
        "Permutation importance and SHAP are computed on the first outer grouped-CV fold, so the explained rows belong only to papers unseen during model fitting.",
    ]
    if lopo_pooled:
        ...
        json.dump(
            {
                ...
                "paper_weighting": weighting_summary.reset_index().to_dict(orient="records"),
                "permutation_importance": permutation.to_dict(orient="records"),
                "shap_ranking": None if shap_ranking is None else shap_ranking.to_dict(orient="records"),
            },
            f, indent=2,
        )
